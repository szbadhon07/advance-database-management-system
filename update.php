...
<?php }
	...
	$c = oci_connect("sz","sz","localhost");
$s1=oci_parse($c,"update book set name='$name',phone='$phone',checkin_date='$startdate',checkout_date='$enddate',room_type='$room_type',
room_price='$roomtype',no_of_room='$room_nos',amount='$amount' where id='".$id."'");
 oci_execute($s1);
if($s1){
...
$id=$_GET['id'];
$c = oci_connect("sz","sz","localhost");
$g=oci_parse($c," select * from book where id= '".$id."' ");
oci_execute($g) ;
$room=oci_fetch_array($g);
}
?>
...
<?php if(isset($_GET['id'])){ ?>
			 <input name="id" type="hidden"  value="<?php if(isset($_GET['id'])){ echo $_GET['id']; }  ?>" /> <?php } ?>
              <input name="startdate1" type="date"  value="<?php if(isset($_POST['startdate1'])){ echo $_POST['startdate1']; } if(isset($_GET['id'])){ echo $room['CHECKIN_DATE']; }  ?>" /></td>          </tr>
          <tr>
